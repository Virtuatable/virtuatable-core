RSpec.describe Core::Models::Monitoring::Service do
  ...
  describe :scopes do
    describe :active do
      ...
      it 'has a scope giving the currently active instances' do
        expect(Core::Models::Monitoring::Service.active.count).to be 1
      end
      it 'returns the currently active instances' do
        expect(Core::Models::Monitoring::Service.active.first.key).to eq 'test.service'
      end
    end
    describe :inactive do
      ...
      it 'has a scope giving the currently inactive instances' do
        expect(Core::Models::Monitoring::Service.inactive.count).to be 1
      end
      it 'returns the currently inactive instances' do
        expect(Core::Models::Monitoring::Service.inactive.first.key).to eq 'test.service'
      end
    end
  end
